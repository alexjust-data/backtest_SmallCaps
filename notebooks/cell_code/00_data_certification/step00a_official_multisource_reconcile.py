# STEP 00A - OFFICIAL MULTISOURCE RECONCILIATION (SEC + optional NASDAQ/NYSE local feeds)
...
# Optional local feeds. Leave as-is if you already placed official files there.
NASDAQ_GLOB = "data/external/nasdaq/**/*.*"
NYSE_GLOB = "data/external/nyse_cta/**/*.*"

ENFORCE_OFFICIAL_DOMAINS = False
ALLOWED_DOMAINS = ""  # Example: "sec.gov,www.sec.gov,nasdaq.com,www.nasdaq.com,nyse.com,ftp.nyse.com"

RUN_ROOT = PROJECT_ROOT / "runs" / "data_quality" / "00_data_certification" / "official_multisource"
RUN_DIR = RUN_ROOT / f"step00a_official_multisource_{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}"
...
print("[00A] Ejecutando reconciliacion multisource...")
print("[00A] Comando:", " ".join(cmd))
subprocess.run(cmd, check=True, cwd=str(PROJECT_ROOT))

summary = json.loads(OUT_SUMMARY_JSON.read_text(encoding="utf-8"))
cov = summary.get("coverage_summary", {})
print("[00A] RUN_DIR:", RUN_DIR)
print("[00A] Coverage summary:", cov)
print("[00A] Outputs:")
