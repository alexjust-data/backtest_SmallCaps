...
while ($true) {
  ...
  Write-Host "agent03-live-fast" -ForegroundColor Cyan
  Write-Host "time: $now"
  ...
  if (!(Test-Path $statusJson)) {
    Write-Host "live_status no existe aun. Esperando a que Agent02 escriba snapshot..." -ForegroundColor Yellow
  ...
  try {
    ...
    Write-Host "No se pudo leer/parsear live_status JSON." -ForegroundColor Red
  }
